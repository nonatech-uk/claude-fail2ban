...
- `claude_fail2ban/` — the importable package
  ...
  - `sources/` — `Source` ABC + `caddy_json.py` (BAD_PATH / SUSPICIOUS_* regex are inlined here), `mailcow_docker.py` (postfix/dovecot/rspamd via `docker logs --since`) and `mailcow_nginx.py` (combined-log-format auth-surface parser, imports `BAD_PATH_PATTERNS`/`ua_family` from `caddy_json`). The shared `_docker_logs.py` helper handles cursor management for both mailcow sources.
  - `actions/` — `Action` ABC + `fail2ban_client.py`, `mailcow_api.py` (POSTs `attr.blacklist` to `/api/v1/edit/fail2ban`).
...
- **Mailcow docker logs are NOT in journald** on stock installs. The docker daemon uses the `json-file` log driver, so `journalctl _SYSTEMD_UNIT=mailcowdockerized-…` is empty. `_docker_logs.py` shells out to `docker logs --since=<rfc3339>` instead and stores per-container cursors under `state["docker_cursors"]`. If a future host changes the daemon log driver to journald, that's the time to write a sibling source.
