...
# to reject obvious junk so "valid mailbox vs unknown" stays meaningful.
_EMAIL_RE = re.compile(
    r"^[A-Za-z0-9._%+-]+@[A-Za-z0-9](?:[A-Za-z0-9-]*[A-Za-z0-9])?"
    r"(?:\.[A-Za-z0-9](?:[A-Za-z0-9-]*[A-Za-z0-9])?)+$"
)

# Per-flavour recipient extractors. Each returns the first matched address,
# or None. Patterns prefer recipient-context tokens over sender ones so we
# don't accidentally surface `from=<...>` as the "target".
_POSTFIX_RCPT_RES = (
    re.compile(r"\bto=<([^<>\s]+@[^<>\s]+)>"),
    # NOQUEUE reject lines: `Recipient address rejected: ...; <user@domain>`
    re.compile(r"Recipient address rejected[^<]*<([^<>\s]+@[^<>\s]+)>"),
    # Generic `RCPT from ... <user@domain>` as a last resort.
    re.compile(r"\bRCPT\b[^<]*<([^<>\s]+@[^<>\s]+)>"),
)
_DOVECOT_RCPT_RES = (
...
_RSPAMD_RCPT_RES = (
    re.compile(r"mime_rcpts?:\s*<?([^<>\s,;]+@[^<>\s,;]+)>?"),
    re.compile(r"\brcpts?:\s*<?([^<>\s,;]+@[^<>\s,;]+)>?"),
)

_FLAVOUR_RCPT_RES = {
    "postfix": _POSTFIX_RCPT_RES,
    "dovecot": _DOVECOT_RCPT_RES,
...
def _extract_target(flavour: str, raw: str) -> tuple[str | None, str | None]:
    ...
    for pat in _FLAVOUR_RCPT_RES.get(flavour, ()):
        ...
        if not m:
            continue
        addr = m.group(1).strip().rstrip(".,;:>").lower()
