...
def _build_provider(spec: dict) -> LLMProvider:
    ...
    if typ == "ollama_native":
        calibration_key = spec.get("calibration", "")
        if calibration_key and calibration_key not in prompts.CALIBRATIONS:
            raise ValueError(
                f"unknown calibration '{calibration_key}' "
                f"(known: {sorted(prompts.CALIBRATIONS)})"
            )
        return OllamaNativeProvider(
            ...
            prompt_suffix=prompts.CALIBRATIONS.get(calibration_key, ""),
        )
